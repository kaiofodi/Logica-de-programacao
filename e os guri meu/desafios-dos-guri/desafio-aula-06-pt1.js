...
/*
let candidato = {
    Nome: guri.question('Diga seu nome: '),
    Idade: guri.questionInt('Qual a sua idade: '),
    Experiencia: guri.questionInt('Tem quantos anos de experiencia: '),
    EnsinoSuperior: guri.keyInYN('Possui ensino superior: '),
    CursoTecnico: guri.keyInYN('Possui curso tecnico: '),
    ResidenoBrasil: guri.keyInYN('Voce mora no Brasil: ')
};
...
if(candidatoAprovado){
    console.log('Parabens! Voce esta aprovado(a) para a proxima fase');
} else {
    console.log('Infelizmente, seu perfil nao atendeu aos requisitos dessa vaga.');
...
// → Seu código aqui:
/*
let temperatura = guri.questionFloat('Diga a temperatura atual em Celsius: ')
let ambienteInterno = guri.keyInYN('Esta em um ambiente fechado: ')
...
if(temperatura < 10){
    ...
    console.log('Um inferno');
}

let precisaArCondicionado =
temperatura > 28 && !ambienteInterno
temperatura > 28 && ambienteInterno

if(precisaArCondicionado){
    console.log('Não, porque já está fora');
} else {
    console.log('Sim, precisa de ar condicionado');
}
console.log("_______________________________");
*/
...
let usuarioAcademia = {
    Nome: guri.question('Digite seu nome: '),
    Idade: guri.questionInt('Diga sua idade: '),
    Estudante: guri.keyInYN('Estuda: '),
    PIX: guri.keyInYN('Realizara o pagamento no PIX: '),
    Planos: guri.questionInt(`
    - Plano 1: R$ 120.00
    - Plano 2: R$ 150.00
    - Plano 3: R$ 180.00
    - Plano 4: R$ 200.00
    Escolha qual plano deseja: `)
}

while( usuarioAcademia.Planos < 1 || usuarioAcademia.Planos > 4){
...
let desconto =
descontoEstudante = usuarioAcademia.Estudante && usuarioAcademia.Idade < 25;
descontoIdoso = usuarioAcademia.Idade >= 60;
descontoPix = usuarioAcademia.PIX && !descontoEstudante && !descontoIdoso
percentual = descontoEstudante ? 10 : descontoIdoso ? 15 : descontoPix ? 5 : 0
