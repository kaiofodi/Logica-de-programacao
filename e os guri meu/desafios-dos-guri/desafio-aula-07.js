...
/*
let IMCUsuario = {
    nome: guri.question('Qual seu nome: '),
    peso: guri.questionFloat('Qual seu peso: '),
    altura: guri.questionFloat('Qual a sua altura:'),
    calculoIMC: [],
};

let calculoIMC = IMCUsuario.peso / (IMCUsuario.altura.toFixed(2) ** 2)

if(IMCUsuario.calculoIMC < 18.5){
    console.log('Abaixo do peso.');
} else if(IMCUsuario.calculoIMC >= 18.5 && IMCUsuario.calculoIMC < 25){
    console.log('Peso normal.');
} else if(IMCUsuario.calculoIMC >= 25 && IMCUsuario.calculoIMC < 30){
    console.log('Sobrepeso.');
} else if(IMCUsuario.calculoIMC >= 30 && IMCUsuario.calculoIMC < 35){
    console.log('Maumau.');
} else if(IMCUsuario.calculoIMC >= 35 && IMCUsuario.calculoIMC < 40){
    console.log('Gordao da XJ');
} else if(IMCUsuario.calculoIMC > 40){
    console.log('Gordao da Outlet. Isso e preocupante...');
}

IMCUsuario.classificacao = calculoIMC
IMCUsuario.calculoIMC.push(calculoIMC)
console.log(IMCUsuario);
console.table(IMCUsuario);

console.log(`Ola ${IMCUsuario.nome}! Seu IMC e de ${IMCUsuario.calculoIMC} - ${IMCUsuario.classificacao}`);
*/
...
let senhaUsuario = guri.questionInt('Digite a sua senha: ')

if(senhaUsuario = conta.senha){
    console.log('senha inorreta. tentativa de golpe.');
}else{
    let saque = guri.questionInt('Qual valor deseja sacar: R$');
} 
